...
export default function UserTable() {
  ...
  const handleEditSubmit = async () => {
    ...
    await fetch(`/api/resign-users/${selectedUser.id}`, {
      method: "PUT",
  ...
  const handleDelete = async (id: number) => {
    await fetch(`/api/resign-users?id=${id}`, {
      method: "DELETE",
    });
    location.reload();
  };
            ...
            <tr key={u.id} className="hover:bg-gray-100">
              ...
              <td className="flex gap-2 p-2 border">
                ...
                >
                 ...
                 {showModal && (
                <AddUserModal onClose={() => setShowModal(false)} initialData={editUser} />)}
                <button
